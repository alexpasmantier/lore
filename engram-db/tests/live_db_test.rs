//! Test against the real ~/.engram/memory.db to verify migration,
...
//! Run with: cargo test -p engram-db --test live_db_test -- --ignored

use engram_db::fragment::now_unix;
use engram_db::relevance::compute_relevance;
use engram_db::{EngramDb, Storage};
use std::path::PathBuf;

fn live_db_path() -> PathBuf {
    let home = std::env::var("HOME").unwrap();
    PathBuf::from(format!("{}/.engram/memory.db", home))
}

fn open_live_db() -> EngramDb {
