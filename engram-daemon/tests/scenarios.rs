...
use engram_daemon::config::ConsolidationConfig;
use engram_daemon::consolidation;
use engram_daemon::ingestion::{ExtractedKnowledge, ExtractedNode, ExtractedTopicEntry};
use engram_daemon::parser::parse_jsonl_line;
use engram_db::edge::EdgeKind;
use engram_db::fragment::{now_unix, Fragment};
use engram_db::relevance::{compute_relevance, MIN_RELEVANCE_THRESHOLD};
use engram_db::storage::Storage;
...
fn test_db() -> EngramDb {
    ...
    EngramDb::new_without_embeddings(storage)
}
...
fn ingest_all(db: &EngramDb) {
    engram_daemon::ingestion::store_knowledge(
        db,
    ...
    .unwrap();
    engram_daemon::ingestion::store_knowledge(db, &extraction_chatter(), Some("session-chatter"))
        .unwrap();
}

/// Parse a fixture JSONL string and return the turns.
fn parse_fixture(jsonl: &str) -> Vec<engram_daemon::parser::ConversationTurn> {
...
fn importance_is_set_correctly_from_extraction() {
    let db = test_db();
    engram_daemon::ingestion::store_knowledge(&db, &extraction_rust_errors(), Some("test"))
        .unwrap();
...
fn temporal_edges_link_sequential_siblings() {
    ...
    engram_daemon::ingestion::store_knowledge(&db, &extraction_async_patterns(), Some("test"))
        .unwrap();
...
fn empty_extraction_stores_nothing() {
    let db = test_db();
    let count =
        engram_daemon::ingestion::store_knowledge(&db, &extraction_chatter(), Some("chatter"))
            .unwrap();
    assert_eq!(count, 0);
...
fn existing_topic_is_augmented_not_duplicated() {
    ...
    engram_daemon::ingestion::store_knowledge(&db, &extraction_rust_errors(), Some("session-1"))
        .unwrap();
    ...
    engram_daemon::ingestion::store_knowledge(&db, &augmentation, Some("session-2")).unwrap();
...
fn hallucinated_topic_id_creates_new_topic() {
    ...
    engram_daemon::ingestion::store_knowledge(&db, &knowledge, Some("test")).unwrap();
...
async fn knowledge_from_different_sessions_builds_unified_graph() {
    ...
    engram_daemon::ingestion::store_knowledge(&db, &extraction_debugging(), Some("session-2"))
        .unwrap();
...
fn superseded_knowledge_is_invisible_but_retained() {
    ...
    engram_daemon::ingestion::store_knowledge(&db, &extraction_rust_errors(), Some("v1")).unwrap();
    ...
    engram_daemon::ingestion::store_knowledge(&db, &updated, Some("v2")).unwrap();
