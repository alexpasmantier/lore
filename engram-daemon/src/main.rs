...
#[command(
    name = "engram-daemon",
    about = "Background daemon for engram memory system"
)]
struct Cli {
    #[command(subcommand)]
    command: Command,
    ...
    /// Path to config file
    #[arg(long, default_value = "~/.engram/config.toml")]
    config: String,
}
...
fn pid_file() -> PathBuf {
    ...
    PathBuf::from(home).join(".engram").join("daemon.pid")
}
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::from_default_env()
                .add_directive("engram_daemon=info".parse().unwrap()),
        )
        .init();
...
async fn run_foreground(config: Config) -> Result<(), Box<dyn std::error::Error>> {
    ...
    tracing::info!("Engram daemon started (PID: {})", pid);
