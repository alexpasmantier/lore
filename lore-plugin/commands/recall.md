...
allowed-tools: ["mcp__plugin_lore_memory__query_memory", "mcp__plugin_lore_memory__explore_memory", "mcp__plugin_lore_memory__traverse_memory", "mcp__plugin_lore_memory__list_roots"]
---
...
1. If no specific topic is given, use `list_roots` to show what's in memory (sorted by relevance — most active first)
...
4. Present the results in a clear, organized way — show the hierarchy from root → concepts → facts
