...
fn daemon_binary() -> PathBuf {
    let home = std::env::var("HOME").unwrap_or_default();
    let local = PathBuf::from(&home).join(".local/bin/lore-daemon");
    if local.exists() {
        return local;
    }
...
fn view_logs() {
    let home = std::env::var("HOME").unwrap_or_default();
    let log_path = format!("{}/.lore/daemon.log", home);
    ...
    eprintln!("lore-tray: could not find a terminal emulator to show logs");
}
